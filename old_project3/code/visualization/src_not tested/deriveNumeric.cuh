...
__global__ void deriveNumeric(float *d_vx, float *d_vy, float *d_refdepthImg,float *d_residual,float *d_jacobif,int w, int h,  float fx, float fy, float cx, float cy, float *rot, float *t, float *JtJ_final,float *Jtb_final) //float *d_display,
{	
        //int j = 0;
	int x = threadIdx.x + blockIdx.x*blockDim.x;
	int y = threadIdx.y + blockIdx.y*blockDim.y;
	if(x<w && y<h)
	{
		...
		{	
			...
			//d_display[idx] = d_jacobif[idx*6+1];
		}
	}	
}
