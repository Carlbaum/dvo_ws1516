...
bool depthToVertexMap(const Eigen::Matrix3d &K, const cv::Mat &depth, cv::Mat &vertexMap)
{
    int w = depth.cols;
    int h = depth.rows;
    float cx = K(0, 2);
...
void transformVertexMap(const Eigen::Matrix3d &R, const Eigen::Vector3d &t, cv::Mat &vertexMap)
{
    ...
    for (int y = 0; y < h; ++y)
    {
        for (int x = 0; x < w; ++x)
        {
            ...
            if (pt.val[2] == 0.0 || std::isnan(pt.val[2]))
                continue;
            Eigen::Vector3d ptTf(pt.val[0], pt.val[1], pt.val[2]);
            ptTf = R * ptTf + t;
...
bool savePlyFile(const std::string &filename, const std::vector<Eigen::Vector3d> &pts, const std::vector<Eigen::Vector3d> &colors)
{
    if (pts.empty())
        return false;
...
bool savePlyFile(const std::string &filename, const cv::Mat &color, const cv::Mat &vertexMap)
{
    ...
    for (int y = 0; y < vertexMap.rows; ++y)
    {
        for (int x = 0; x < vertexMap.cols; ++x)
        {
            ...
            if (pt.val[2] == 0.0 || std::isnan(pt.val[2]))
                continue;
            pts.push_back(Eigen::Vector3d(pt.val[0], pt.val[1], pt.val[2]));

            cv::Vec3b c = color.at<cv::Vec3b>(y, x);
            colors.push_back(Eigen::Vector3d(c.val[2], c.val[1], c.val[0]));
        }
    }
