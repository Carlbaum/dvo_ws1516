...
__global__ void downSample(float *imgOut, float *imgIn,int h, int w)
{
                ...
                std::vector<float *> &depthCurPyramid, int w, int h)
{
   int width = w, height = h;
	...
	//Output refernce depth image
 	...
 	downSample <<<grid,block>>> (d_curOut,grayCurPyramid[i-1],height,width); CUDA_CHECK;  	//Gray current image
	downSample <<<grid,block>>> (d_refimgOut,grayRefPyramid[i-1],height,width); CUDA_CHECK;  	//Reference image
